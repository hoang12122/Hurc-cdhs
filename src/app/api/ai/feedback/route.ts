...
export async function POST(req: Request) {
  try {
    ...
    const learningPolicy = resolveContinuousLearningPolicy();
    if (!checkRateLimit(
      `ai_feedback:${user.id}`,
    ...
    return NextResponse.json({
      success: true,
      feedbackId,
      learningState: learningPolicy.enabled ? 'QUEUED_FOR_GOVERNED_REVIEW' : 'COLLECTED_ONLY',
