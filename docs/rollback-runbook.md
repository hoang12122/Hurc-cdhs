...
| **3. Kiểm Thử Sức Bền (Smoke)** | Tất cả các bước Smoke Test trả về mã `2xx/3xx` | Một bước smoke thất bại hoặc log chứa crash signature | 🛑 **NO-GO**: **KÍCH HOẠT ROLLBACK LẬP TỨC** |

> [!IMPORTANT]
> **QUY TẮC PHÁN QUYẾT BẤT BIẾN (GO/NO-GO RULES):**
>
> * **GO:** Chỉ được cấp phép khi và chỉ khi **tất cả** các bước kiểm nghiệm của Smoke Test Core (và AI nếu có) hoàn tất thành công 100% không phát sinh lỗi.
> * **NO-GO:** Ngay lập tức từ chối và kích hoạt quy trình rollback khẩn cấp nếu **bất kỳ** một bước smoke nhỏ nào trả về thất bại (non-zero exit code) hoặc phát hiện signature của sự cố sập luồng (Crash Loop).

Để đảm bảo tính nghiêm ngặt và tự động hóa khép kín:

> [!WARNING]
> Quy tắc khép kín: **Tuyệt đối không để xảy ra tình trạng "Build thành công nhưng runtime thất bại" (Build Pass but Runtime Fail). Tiến trình triển khai chỉ được coi là thành công khi và chỉ khi vượt qua thành công cổng kiểm định Smoke Test tự động tại môi trường mô phỏng (Staging/CI-Runner).**
