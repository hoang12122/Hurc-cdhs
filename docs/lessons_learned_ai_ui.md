...
- **Cách khắc phục & phòng ngừa:**
  ...
  - Tạo tập lệnh bảo vệ [guard-install.sh](file:///d:/Hurc1CRM-main/Hurc-cdhs/scripts/guard-install.sh) để phát hiện ngữ cảnh CI/Deploy và đón chặn hoàn toàn hành vi chạy `npm install` sai lệch.
...
- **Cách khắc phục & phòng ngừa:**
  - Loại bỏ hoàn toàn cơ chế fallback `npm install` trong tệp triển khai [deploy.ps1](file:///d:/Hurc1CRM-main/Hurc-cdhs/deploy.ps1).
