...
- **Chi tiết thực hiện:**
  ...
  - Giao diện có tích hợp nút `Accept (Check)` and `Reject (X)`, cho phép kỹ sư con người huấn luyện ngược lại AI (Human-in-the-loop). Mỗi lần bấm Reject, AI sẽ lưu log để tự hạ trọng số của đề xuất đó trong tương lai.
- **Kết quả / Output:**
